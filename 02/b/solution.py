...
for action in course:
    report: str = f'executing command {action["command"]} {action["distance"]} from ({horizontal_pos}, {vertical_pos}, {aim}):'

    match action['command']:
        case 'forward':
            horizontal_pos += action['distance']
            vertical_pos += aim * action['distance']
        case 'up':
            aim -= action['distance']
        case 'down':
            aim += action['distance']

    result.append(f'{report} new position ({horizontal_pos}, {vertical_pos}, {aim})')
