...
def calculate_antinode(row1, col1, row2, col2):
    _row = row1 + (2 * (row2 - row1))
    _col = col1 + (2 * (col2 - col1))
...
for _k, v in output.items():
    ...
    antinodes = set()
    for perm in perms:
        candidate = calculate_antinode(*perm[0], *perm[1])
        if is_valid(*candidate, data):
            all_antinodes.add(candidate)
